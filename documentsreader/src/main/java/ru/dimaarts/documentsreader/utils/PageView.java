...
import ru.dimaarts.documentsreader.tasks.AsyncTask;
import ru.dimaarts.documentsreader.tasks.CancellableAsyncTask;
import ru.dimaarts.documentsreader.tasks.CancellableTaskDefinition;
...
public abstract class PageView extends ViewGroup {
	...
	public void init(Context c, FilePicker.FilePickerSupport filePickerSupport, MuPDFCore core, Point parentSize, Bitmap sharedHqBm) {
		mParentSize = parentSize;
		setBackgroundColor(Color.TRANSPARENT);
		mEntireBm = Bitmap.createBitmap(parentSize.x, parentSize.y, Config.ARGB_8888);
		mPatchBm = sharedHqBm;
	...
	public void blank(int page) {
		...
		mBackgroundView.setBackgroundColor(BACKGROUND_COLOR);
		setBackgroundColor(Color.TRANSPARENT);
	}

	public void setPage(int page, PointF size, boolean canDraw) {
		// Cancel pending render task
		if (mDrawEntire != null) {
			mDrawEntire.cancel();
			mDrawEntire = null;
		...
		size.set(size.x-getPaddingLeft()-getPaddingRight(), size.y-getPaddingTop()-getPaddingBottom());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(mBackgroundView !=null) {
			mBackgroundView.measure(View.MeasureSpec.AT_MOST, View.MeasureSpec.AT_MOST);
		}
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (mEntire != null) {
			...
			mEntire.layout(getPaddingLeft(), getPaddingTop(), w+getPaddingLeft(), h+getPaddingTop());
		}

		if(mBackgroundView !=null) {
			mBackgroundView.layout(getPaddingLeft(), getPaddingTop(), w+getPaddingLeft(), h+getPaddingTop());
		}

		if (mSearchView != null) {
			mSearchView.layout(getPaddingLeft(), getPaddingTop(), w+getPaddingLeft(), h+getPaddingTop());
		}
	...
	public void updateHq(boolean update) {
		Rect viewArea = new Rect(getLeft()+getPaddingLeft(),getTop()+getPaddingTop(),getRight()-getPaddingRight(),getBottom()-getPaddingBottom());
		if (viewArea.width() == mSize.x || viewArea.height() == mSize.y) {
			...
			patchArea.offset(-viewArea.left+getPaddingLeft(), -viewArea.top+getPaddingTop());
			...
			if (completeRedraw)
				task = getDrawPageTask(mPatchBm, patchViewSize.x, patchViewSize.y,
								patchArea.left-getPaddingLeft(), patchArea.top-getPaddingTop(),
